...
Although this ignores places where the field is defined in a function, as its hard to distinguish if the comment  functions that aren't called by the module itself before it returns, so the following isn't allowed:
